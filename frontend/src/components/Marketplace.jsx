...
export default function Marketplace() {
  ...
  const fetchMarketplaceKeys = async () => {
    setLoading(true);
    try {
      const res = await apiKeysAPI.getMarketplace(
  ...
  const handlePurchase = async (keyId) => {
    if (
      !window.confirm(
        "Are you sure you want to purchase this API key? This will use your active subscription credits.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.error(
        error.response?.data?.detail ||
          "Purchase failed. Ensure you have an active Pro/Max subscription.",
      );
    } finally {
      setPurchasing(false);
    }
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {availableKeys.map((apiKey) => (
            <div
              key={apiKey.id}
              className="group bg-white border border-gray-100 rounded-3xl p-8 hover:shadow-2xl hover:shadow-indigo-100 transition-all border-b-4 hover:border-indigo-600"
            >
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-700">
                      {apiKey.rate_limit_per_minute.toLocaleString()} / min
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
