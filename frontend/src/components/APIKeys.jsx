...
export default function APIKeys({ apiKeys, setApiKeys }) {
  ...
  const handleToggle = async (id) => {
    try {
      ...
      toast.success(
        response.data.is_active ? "API key activated" : "API key deactivated",
      );
    } catch (error) {
  ...
  const handleRotate = async (id) => {
    if (
      !confirm("Rotate this key? The current key will remain valid for 7 days.")
    )
      return;
    try {
        ...
        <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Manage your keys for external service integration.
            </p>
          </div>
          {!user?.is_superuser && (
                  ...
                  <div className="flex-1">
                    ...
                    {key.expires_at && (
                      <p className="text-xs font-bold text-red-400 mt-2 flex items-center gap-1">
                        <Zap className="w-3 h-3" /> Access Expires:{" "}
