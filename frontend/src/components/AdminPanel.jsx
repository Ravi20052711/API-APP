...
import { Key, Plus, Trash, ShieldCheck, Users, ShoppingBag, Database, Cpu, Globe, Zap, ToggleRight, ToggleLeft } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      toast.success(newKey.is_paid ? 'Marketplace key published!' : 'Internal key created!');
      setShowCreate(false);
      fetchData();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if(!confirm('Are you sure you want to delete this key? This action is permanent.')) return;
    try {
      ...
      <div className="flex bg-white p-1 rounded-2xl shadow-sm border border-gray-100 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-8 py-3 rounded-xl font-bold transition-all ${
            activeTab === 'keys' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:bg-gray-50'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-8 py-3 rounded-xl font-bold transition-all ${
            activeTab === 'users' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:bg-gray-50'
          }`}
        >
        ...
        <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            <h2 className="text-2xl font-black text-gray-900 flex items-center gap-3">
              ...
              {activeTab === 'keys' ? 'Marketplace Inventory' : 'Registered Platform Users'}
            </h2>
            <p className="text-gray-500 mt-1">Management console for system-wide resources.</p>
          </div>
          {activeTab === 'keys' && (
            <button
            ...
            <form onSubmit={handleCreate} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-black text-indigo-900 uppercase">Key Name</label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-indigo-900 uppercase">Category</label>
                  <select
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-indigo-900 uppercase">Price (INR)</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-gray-400">₹</span>
                    <input
                      type="number"
                      value={newKey.price}
                      onChange={e => setNewKey({...newKey, price: Number(e.target.value)})}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white/50 p-6 rounded-2xl border-2 border-dashed border-indigo-100">
                <div className="space-y-2">
                  <label className="text-xs font-black text-indigo-900 uppercase tracking-widest flex items-center gap-2">
                    <Globe className="w-3 h-3" /> Real API Endpoint (Upstream URL)
                  </label>
                  <input
                    type="url"
                    placeholder="https://api.openai.com/v1"
                    value={newKey.upstream_url}
                    onChange={e => setNewKey({...newKey, upstream_url: e.target.value})}
                    className="w-full px-4 py-3 rounded-xl border-2 border-indigo-100 focus:border-indigo-600 outline-none"
                  />
                  <p className="text-[10px] text-gray-400">The gateway will proxy requests to this URL.</p>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-black text-indigo-900 uppercase tracking-widest flex items-center gap-2">
                    <ShieldCheck className="w-3 h-3" /> Secret Upstream Key (Masked)
                  </label>
                  <input
                    type="password"
                    placeholder="Enter the real secret key"
                    value={newKey.upstream_key}
                    onChange={e => setNewKey({...newKey, upstream_key: e.target.value})}
                    className="w-full px-4 py-3 rounded-xl border-2 border-indigo-100 focus:border-indigo-600 outline-none"
                  />
                  <p className="text-[10px] text-gray-400">This key will never be shown but used for proxying.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-center gap-4 bg-white p-4 rounded-xl border-2 border-indigo-100">
                  <div className="flex-1">
                    <p className="text-sm font-bold text-gray-900">Paid Marketplace Listing</p>
                    <p className="text-xs text-gray-500">Enable to publish this key to the public marketplace.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setNewKey({...newKey, is_paid: !newKey.is_paid})}
                    className={`w-14 h-8 rounded-full transition-colors relative ${newKey.is_paid ? 'bg-indigo-600' : 'bg-gray-200'}`}
                  >
                    <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all ${newKey.is_paid ? 'left-7' : 'left-1'}`} />
                  </button>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-black text-indigo-900 uppercase">Target User ID (Optional)</label>
                  <input
                    type="number"
                    placeholder="e.g. 5 (Leave empty for Marketplace)"
                    value={newKey.target_user_id}
                    onChange={e => setNewKey({...newKey, target_user_id: e.target.value})}
                    className="w-full px-4 py-3 rounded-xl border-2 border-indigo-100 focus:border-indigo-600 outline-none"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button type="submit" className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-lg hover:bg-indigo-700 transition-all">
                  Confirm and Create Key
                </button>
                <button type="button" onClick={() => setShowCreate(false)} className="px-8 py-4 bg-white text-gray-500 rounded-2xl font-bold border-2 border-gray-100">
                  Cancel
                </button>
              </div>
            </form>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  {activeTab === 'keys' ? (
                    <>
                      <th className="px-8 py-4 text-left text-xs font-black text-gray-400 uppercase tracking-widest">Resource Name</th>
                    ...
                    <>
                      <th className="px-8 py-4 text-left text-xs font-black text-gray-400 uppercase tracking-widest">UID</th>
                      <th className="px-8 py-4 text-left text-xs font-black text-gray-400 uppercase tracking-widest">User Profile</th>
                      <th className="px-8 py-4 text-left text-xs font-black text-gray-400 uppercase tracking-widest">Email</th>
                      <th className="px-8 py-4 text-left text-xs font-black text-gray-400 uppercase tracking-widest">Last Login</th>
                      <th className="px-8 py-4 text-left text-xs font-black text-gray-400 uppercase tracking-widest">Status</th>
                      <th className="px-8 py-4 text-right text-xs font-black text-gray-400 uppercase tracking-widest">Joined</th>
                    </>
                  )}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {activeTab === 'keys' ? keys.map(key => (
                  <tr key={key.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-gray-900">{key.name || 'API Key'}</p>
                          <p className="text-xs font-mono text-gray-400">{key.key.substring(0, 12)}...</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                    ...
                    <td className="px-8 py-5 font-black text-gray-900">₹{key.price.toLocaleString()}</td>
                    <td className="px-8 py-5">
                      {key.user_id ? (
                        <span className="flex items-center gap-1.5 text-green-600 font-bold text-xs">
                          <Users className="w-3.5 h-3.5" /> Assigned (UID: {key.user_id})
                        </span>
                      ) : (
                        <span className="flex items-center gap-1.5 text-indigo-600 font-bold text-xs">
                          <ShoppingBag className="w-3.5 h-3.5" /> In Marketplace
                        </span>
                      )}
                    </td>
                    <td className="px-8 py-5 text-right">
                      <button onClick={() => handleDelete(key.id)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all">
                  ...
                  <tr key={u.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-8 py-5 font-mono text-xs text-indigo-600 font-bold">#{u.id}</td>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
