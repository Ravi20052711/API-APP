            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
              ...
              <div className="flex gap-4">
                <div className="bg-white/10 backdrop-blur-xl p-6 rounded-3xl border border-white/20">
                  <p className="text-xs font-bold text-indigo-100 uppercase mb-1">
                    Current Plan
                  </p>
                  <p className="text-2xl font-black">
                    {user?.is_superuser ? "Enterprise Admin" : "Pro Member"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 space-y-8">
              <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-black text-gray-900">
                      {100 - (stats?.error_rate || 0)}%
                    </span>
                  </div>
                  <div className="pt-4 border-t border-gray-50">
                    <p className="text-xs text-gray-400 leading-relaxed italic">
                      Your account is in good standing. All gateway endpoints
                      are reachable.
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-8 rounded-[2.5rem] text-white shadow-xl">
                ...
                <p className="text-gray-400 text-sm leading-relaxed mb-8">
                  Unlock specialized AI and Automation keys in the marketplace.
                </p>
                <Link
                  to="/marketplace"
                  className="block w-full py-4 bg-indigo-600 rounded-2xl text-center font-black hover:bg-indigo-500 transition-all"
                >
                  Browse Marketplace
                </Link>
              </div>
...
function APIKeysPage() {
  ...
  return (
    <PageLayout title="Integrations" maxWidth="max-w-5xl">
...
function AdminPage() {
  ...
  return (
    <PageLayout title="Admin Hub" maxWidth="max-w-[1400px]">
      <AdminPanel />
    </PageLayout>
  );
}
...
function UsagePage() {
  return (
    <PageLayout title="Analytics" maxWidth="max-w-7xl">
      <UsageStats />
    </PageLayout>
  );
}
...
function WebhooksPage() {
  return (
    <PageLayout title="Webhooks" maxWidth="max-w-5xl">
      <Webhooks />
    </PageLayout>
  );
}
...
function BillingPage() {
  return (
    <PageLayout title="Subscription" maxWidth="max-w-7xl">
      <BillingPlans />
    </PageLayout>
  );
}
...
function MarketplacePage() {
  return (
    <PageLayout title="Marketplace" maxWidth="max-w-7xl">
      <Marketplace />
    </PageLayout>
  );
}
    ...
    <div className="min-h-screen bg-white font-sans overflow-hidden">
      ...
      <nav className="relative z-50 p-8 flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-8">
          <Link
            to="/login"
            className="text-sm font-black text-gray-500 hover:text-indigo-600 transition-colors uppercase tracking-widest"
          >
            Login
          </Link>
          <Link
            to="/register"
            className="px-8 py-3 bg-gray-900 text-white rounded-2xl font-black text-sm hover:bg-indigo-600 transition-all shadow-xl shadow-gray-200"
          >
            Get Started
          </Link>
        </div>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-8 pt-20 pb-32 flex flex-col items-center text-center">
        ...
        <p className="text-xl md:text-2xl text-gray-500 max-w-3xl leading-relaxed mb-16 font-medium">
          Mask, distribute, and monetize your API keys with enterprise-grade
          security. Automated billing, real-time usage tracking, and
          instantaneous proxying.
        </p>

        <div className="flex flex-col md:flex-row gap-6">
          <Link
            to="/register"
            className="group px-12 py-6 bg-indigo-600 text-white rounded-[2rem] font-black text-xl shadow-2xl shadow-indigo-300 hover:bg-indigo-700 transition-all flex items-center gap-4"
          >
